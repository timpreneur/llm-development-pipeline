...
Shared floors and optimization targets for work produced by `dev-pipeline-plan` and `dev-pipeline-build`. Read once per session before writing a plan or writing code. Other skills in this plugin (ideate, ship, research, llm-ops) do not produce implementation code and are not bound by this file.
...
See `dev-pipeline-build/references/plan-issue-template.md` for the filing structure and `dev-pipeline-plan/references/plan-issue-diagnosis.md` for the diagnosis routine.
