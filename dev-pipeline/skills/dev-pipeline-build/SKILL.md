...
If Legal and Marketing wrappers ship in a later version, they'll live as separate skills (`dev-pipeline-build:legal-mode`, `dev-pipeline-build:marketing-mode`). This skill documents the hook points but does not invoke them in v0.1.0.
...
4. Surface to Timm: "Preview live at `<URL>`. `03-build-notes.md` written. Next: open a new Cowork session and paste `dev-pipeline-ship/startup-prompt.md` with `<FEATURE_ID>` = `<actual>` and `<FEATURE_DIR>` = `<actual>`."
...
For cases 2 and 3, the escalation channel is `02-plan-issue.md`, not a direct message. The structured file captures the context Plan needs to diagnose without Build re-explaining; Timm reads it and routes to dev-pipeline-plan. See `references/plan-issue-template.md`.
...
4. Stop. Wait for Timm to either hand you a revised `02-plan.md` (via a dev-pipeline-plan diagnosis session) or a direct instruction.
...
```bash
bash skills/dev-pipeline-build/scripts/smoke.sh
```
