# Close checklist — dev-pipeline-build
...
```bash
bash skills/dev-pipeline-build/scripts/check-build-notes.sh <FEATURE_DIR>/03-build-notes.md
```
...
For every entry listed in `## Lessons captured` with promotion candidate `watch` or `yes`, append a dated entry to `skills/dev-pipeline-build/lessons.md`. Tag by mode. One append per lesson.
