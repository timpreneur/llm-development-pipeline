# Close checklist — dev-pipeline-ideate
...
Use the template at `dev-pipeline-substrate/references/manifest-template.md`. Status table:
...
```bash
bash skills/dev-pipeline-ideate/scripts/check-brief.sh <FEATURE_DIR>/01-brief.md
```
...
> "Brief approved. Feature dir at `<FEATURE_DIR>`. Next: open Claude Code on `<PROJECT>` in plan mode and paste `dev-pipeline-plan/startup-prompt.md` with `<FEATURE_ID>` = `<actual>` and `<FEATURE_DIR>` = `<actual>`."
