...
Reference for `dev-pipeline-plan` Mode 2: diagnosing a `02-plan-issue.md` filed by `dev-pipeline-build`.
...
- *Resolution path*: **do not revise `02-plan.md`.** File `01-brief-issue.md` instead and stop. Bounce back to `dev-pipeline-ideate` for brief revision. Once brief is updated, re-run this diagnosis with the new brief context.
...
When you reclassify D to B, note it explicitly in the Plan Revisions log entry. If you find yourself reclassifying D to B across more than one feature in a month, that is a drift signal: flag it for `dev-pipeline-llm-ops` to consume (the plan-writing protocol itself needs tightening).
...
4. Surface: "Issue traces back to `01-brief.md` scope ambiguity. Filed `01-brief-issue.md`. Need brief revision via `dev-pipeline-ideate` before plan can be revised."
...
Hand `01-brief-issue.md` off to Timm. He resolves via `dev-pipeline-ideate` (brief revision) or direct decision. Once the brief is updated, `dev-pipeline-plan` re-runs this diagnosis with the new brief context.
