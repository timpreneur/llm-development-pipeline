...
if [[ -f "$PUNCHLIST_PATH" ]]; then
  # Use -r not -x so Dropbox-mounted trees (which strip the execute bit)
  # still pass the check. We invoke via `bash` explicitly, so readability
  # is what actually matters.
  if [[ -r "$CHECK_PUNCHLIST" ]]; then
    ...
    record_yellow "check-punchlist.sh not found or not readable at $CHECK_PUNCHLIST — skipping sibling JSON validation"
  fi
else
