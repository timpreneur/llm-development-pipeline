...
```json
{
  ...
  "seeded_by": "dev-pipeline-substrate",
  "entries": [
    {
...
Write policy: only `dev-pipeline-substrate` (on bootstrap or repair) and `dev-pipeline-llm-ops` (on drift checks / re-hash) write to this file.
...
Append-only. Entries are dated. Owned by dev-pipeline-substrate (seed, repairs) and dev-pipeline-llm-ops (everything else).

## 2026-04-19
- substrate: workspace bootstrap (dev-pipeline-substrate v0.1.0). Seeded _meta/skill-hashes.json (12 entries, all placeholder hashes), _meta/CHANGELOG.md, _meta/metrics.md.
