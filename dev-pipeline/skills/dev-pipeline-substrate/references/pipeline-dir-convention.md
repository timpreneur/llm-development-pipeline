...
- The cohort dir is created by `dev-pipeline-research` when it starts. It doesn't exist ahead of time.
...
- Seed format is specified in `dev-pipeline-research/references/inbox-stub-format.md`.
...
Recommendation: move feature dirs here 180 days post-ship. Not implemented in v0.1.0 — deferred as Brief Open Question #5. When implemented, owned by `dev-pipeline-llm-ops` via a scheduled task.
...
When `dev-pipeline-ship` closes with `deploy_state: READY`, it reads the project's `CLAUDE.md` for a `pipeline` block:
...
If not declared, promotion is explicitly N/A and `dev-pipeline-ship` notes this in `04-ship.md`.
...
Per Brief Open Question #2, if `ExitPlanMode` can only write inside the repo (to be verified at implementation time), `dev-pipeline-plan` may instead write `02-plan.md` to `<project>/.pipeline/<feature-id>/02-plan.md` and the manifest parser treats that path as equivalent to `Code/pipeline/<project>/<feature-id>/02-plan.md`. Default is pipeline-dir; repo fallback only if plan mode refuses.
