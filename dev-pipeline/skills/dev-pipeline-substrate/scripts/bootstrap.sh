#!/usr/bin/env bash
#
# dev-pipeline-substrate bootstrap
#
  ...
  cat <<EOF
{
  "version": 1,
  "seeded_at": "$TODAY",
  "seeded_by": "dev-pipeline-substrate $PLUGIN_VERSION",
  "entries": [
    {
      "id": "builtin:review",
      "type": "built-in",
      ...
      "hash": null,
      "hashed_at": null,
      "used_by": ["dev-pipeline-build:code-opt-mode"],
    ...
    },
    {
      ...
      "hash": null,
      "hashed_at": null,
      "used_by": ["dev-pipeline-build:security-mode"],
      "note": "Claude Code built-in; drift detected via CC version bump, not content hash."
    },
    {
    ...
    },
    {
      ...
      "hash": "sha256:seed",
      "hashed_at": null,
      "used_by": ["dev-pipeline-build:accessibility-mode"]
    },
    {
      ...
      "hash": "sha256:seed",
      "hashed_at": null,
      "used_by": ["dev-pipeline-build:ux-polish-mode"]
    },
    {
    ...
    },
    {
      ...
      "hash": "sha256:seed",
      "hashed_at": null,
      "used_by": ["dev-pipeline-build:qa-mode"]
    },
    {
    ...
    },
    {
      ...
      "hash": "sha256:seed",
      "hashed_at": null,
      "used_by": ["dev-pipeline-ideate", "dev-pipeline-ship"]
    },
    {
...
Append-only log of pipeline-level changes. Owned by dev-pipeline-substrate (seed, repairs) and dev-pipeline-llm-ops (everything else).

## $TODAY
- substrate: workspace bootstrap (dev-pipeline-substrate $PLUGIN_VERSION). Seeded _meta/skill-hashes.json, _meta/CHANGELOG.md, _meta/metrics.md. All marketplace-skill hashes are placeholder "sha256:seed" — first dev-pipeline-llm-ops drift pass will replace them with real hashes.
EOF
}
...
Aggregated after each cohort Research session closes. Owned by dev-pipeline-llm-ops.
...
(populated by dev-pipeline-llm-ops after each cohort closes)
EOF
}
...
if [[ " ${present[*]-} " == *"$META_DIR/CHANGELOG.md"* ]]; then
  {
    echo ""
    echo "## $TODAY"
    echo "- substrate: bootstrap re-run (dev-pipeline-substrate $PLUGIN_VERSION). _meta/ already present; no seeds overwritten."
  } >> "$META_DIR/CHANGELOG.md"
fi

echo "dev-pipeline-substrate bootstrap @ $PIPELINE_ROOT"
echo "  created:"
if (( ${#created[@]} )); then
