...
Don't invoke for: planning (that's `pipeline-plan`, Claude Code plan mode), execution (`pipeline-build`), ship, or research. If Timm says "ship" or "plan" or "build", those are different skills.
...
2. Write `<FEATURE_DIR>/00-manifest.md` using the template in `pipeline-substrate/references/manifest-template.md`. Status row for `01-ideate` = `done`, rest `pending`, aggregate = `in_progress`.
...
6. Confirm next step to Timm: "Open Claude Code on `<PROJECT>` in plan mode. Paste `pipeline-plan/startup-prompt.md` with `<FEATURE_ID>`=`<actual>` and `<FEATURE_DIR>`=`<actual>`."
...
- Plan implementation (that's `pipeline-plan`).
...
```bash
bash skills/pipeline-ideate/scripts/smoke.sh
```
