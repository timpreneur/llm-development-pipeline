...
If `01-brief.md` is missing or has `status: pending|in_progress|blocked`, surface to Timm: "No approved brief at `<FEATURE_DIR>/01-brief.md`. Run pipeline-ideate first."
...
4. Confirm next step to Timm: "Open a NEW Claude Code session on `<PROJECT>` (NOT plan mode — execution mode). Paste `pipeline-build/startup-prompt.md` with `<FEATURE_ID>` = `<actual>` and `<FEATURE_DIR>` = `<actual>`. Build is a new session — context does not carry over."
...
```bash
bash skills/pipeline-plan/scripts/smoke.sh
```
