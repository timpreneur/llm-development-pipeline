...
```bash
bash skills/pipeline-plan/scripts/check-plan.sh <FEATURE_DIR>/02-plan.md
```
...
> "Plan approved. `02-plan.md` at `<FEATURE_DIR>`. Next: open a **NEW** Claude Code session on `<PROJECT>` in **execution mode** (not plan mode). Paste `pipeline-build/startup-prompt.md` with `<FEATURE_ID>` = `<actual>` and `<FEATURE_DIR>` = `<actual>`. Build is a new session — context does not carry over, Build re-reads manifest/brief/plan."
