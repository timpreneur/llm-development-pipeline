...
3. **Shape.** Schema v2. Include `schema_version: "2"`, `schema.generator: "pipeline-ship@<version>"`, `feature_id`, `project`, `short_sha`, `full_sha`, `deploy_url`, `deploy_state: "READY"`, `title`, `created_at`, and the `agentic_pass` + `items` blocks. Field-by-field rules in the schema reference.
