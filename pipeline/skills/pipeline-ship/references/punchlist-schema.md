...
| `schema.generator` | string | yes | Format: `pipeline-ship@<version>`. Used for future retirement decisions |
...
Pushes that don't go through pipeline-ship still write V1 files (`<short_sha>.json` without `schema_version`). The web app renders both shapes side-by-side in the project's deploy list. Nothing about this schema breaks the V1 path.
