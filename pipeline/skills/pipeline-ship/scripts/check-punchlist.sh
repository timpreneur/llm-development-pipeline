...
# schema.generator required, format pipeline-ship@<version>
schema_obj = data.get("schema")
if isinstance(schema_obj, dict):
    gen = schema_obj.get("generator")
    if not gen:
        r("schema.generator missing")
    elif not isinstance(gen, str) or not gen.startswith("pipeline-ship@"):
        r(f"schema.generator must match 'pipeline-ship@<version>' (got {gen!r})")
elif "schema" in data:
