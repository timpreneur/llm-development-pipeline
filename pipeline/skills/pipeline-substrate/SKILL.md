...
description: >
  ...
  Also triggers on "bootstrap pipeline," "seed pipeline meta," "set up pipeline workspace," "validate pipeline convention," "check pipeline dir," "fix pipeline manifest," or when any of the session skills (`pipeline-ideate`, `pipeline-plan`, `pipeline-build`, `pipeline-ship`, `pipeline-research`, `pipeline-llm-ops`) need the authoritative path / template / flag contract.
---
...
- `pipeline-llm-ops` is about to run a drift check and needs `_meta/skill-hashes.json` to exist.
...
4. **Defines the `_meta/` contract** — `references/meta-contract.md` specifies what each `_meta/` file contains, who writes to it, and why. `pipeline-llm-ops` operates against this contract.
...
5. Append a single dated line to `_meta/CHANGELOG.md`: `- <YYYY-MM-DD>: pipeline-substrate bootstrap (version <plugin-version>)`.
...
Run when a session skill hands off a feature dir, when `pipeline-llm-ops` audits state, or when the user asks "is my pipeline OK."
...
- `pipeline-ideate` reads `references/session-file-template.md` before writing `01-brief.md` and `00-manifest.md`.
- `pipeline-plan` reads `references/manifest-template.md` when it inserts the `02-plan` row.
- `pipeline-build` reads `references/session-file-template.md` for the frontmatter it writes on `03-build-notes.md`, and consults `references/pipeline-dir-convention.md` for promotion paths.
- `pipeline-ship` reads the promotion-path rules in `references/pipeline-dir-convention.md` + the manifest-close rules.
- `pipeline-research` reads the cohort-dir rules.
- `pipeline-llm-ops` reads `references/meta-contract.md` end to end — it's LLM Ops' home.
...
Append dated entries to `lessons.md` when substrate itself reveals a gap — e.g., a convention that didn't survive contact with a real feature, a `_meta/` shape that `pipeline-llm-ops` couldn't parse. LLM Ops reconciles.
